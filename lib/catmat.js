...
export async function consultarCATMAT(codigo) {
    ...
    try {
        console.log(`[CATMAT] Querying Supabase for code: ${codigo}`);
        ...
        if (error || !item) {
            console.warn(`[CATMAT] Code ${codigo} not found in database.`);
            ...
            return {
                ...
                descricao: 'Descrição não disponível na base',
        ...
        console.log(`[CATMAT] Found in database: ${item.nome}`);
        ...
        return {
            ...
            grupo: null,
            dados_completos: item.dados_completos || item
        };

    } catch (error) {
        console.error('[CATMAT] Database query failed:', error);
        return {
            status: 'OK', // Fail open
            codigo,
