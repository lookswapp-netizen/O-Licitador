...
export async function analyzeWithFlow(description, ca, catmat) {
  ...
  try {
    ...
    // REGEX FALLBACK: If Gemini failed to detect CATMAT, try Regex
    // This ensures we catch codes even if the AI misses them
    if (!geminiAnalysis.detetive_de_codigos?.catmat_br ||
      geminiAnalysis.detetive_de_codigos.catmat_br.includes("Nenhum") ||
      geminiAnalysis.detetive_de_codigos.catmat_br === "N/A") {

      const catmatRegex = /(?:CATMAT|BR|CÓDIGO|CODIGO)[\s:.-]*(\d{6,})/i;
      const match = description.match(catmatRegex);

      if (match && match[1]) {
        console.log(`[GEMINI] Regex Fallback found CATMAT: ${match[1]}`);
        if (!geminiAnalysis.detetive_de_codigos) geminiAnalysis.detetive_de_codigos = {};
        geminiAnalysis.detetive_de_codigos.catmat_br = match[1];
      }
    }
    ...
    // CATMAT Data Enrichment: If Gemini detected a CATMAT but flow didn't populate catmat_data, query local DB
    if (geminiAnalysis.detetive_de_codigos?.catmat_br &&
      geminiAnalysis.detetive_de_codigos.catmat_br !== "Nenhum detectado" &&
      !finalResult.catmat_data) {

      try {
        const catmatCode = geminiAnalysis.detetive_de_codigos.catmat_br;
        console.log(`[GEMINI] Enriching CATMAT data for detected code: ${catmatCode}`);

        const { consultarCATMAT } = await import('./catmat.js');
        const catmatData = await consultarCATMAT(catmatCode);

        if (catmatData && catmatData.status === 'OK') {
          finalResult.catmat_data = catmatData;
          console.log(`[GEMINI] CATMAT data enriched successfully`);
        }
      } catch (error) {
        console.warn(`[GEMINI] Failed to enrich CATMAT data:`, error.message);
        // Non-breaking: Continue without enrichment
      }
    }
