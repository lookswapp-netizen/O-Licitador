...
4. REGRA DO PRODUTO DE REFERÊNCIA:
   - Identifique uma MARCA e MODELO específicos do mercado brasileiro que atendam às especificações técnicas.
   ...
   **QUANDO RETORNAR MARCA/MODELO:**
   ...
   - Priorize marcas líderes de mercado brasileiras (Samsung, LG, Midea, Dell, HP, Makita, Bosch, Volk, Danny, etc.)
   - Exemplo válido: "Ar Condicionado Split Inverter 12000 BTU" → "LG DUAL Inverter S4-W12JA3AA" ou similar
   
   **QUANDO RETORN AR NULL:**
   ...
   **REGRA ANTI-ALUCINAÇÃO:**
   ...
   - Se incerto sobre o modelo exato, use a marca líder + categoria (ex: "Samsung - Ar Condicionado Split 12000 BTU Inverter")
   - NUNCA invente números de modelo fictícios
...
export async function analyzeWithFlow(description, ca, catmat) {
  ...
  try {
    ...
    try {
      ...
      const prompt = `${SYSTEM_PROMPT}\n\nDescri ção Técnica: "${description}"`;
